...
(() => {
    ...
    function showErrorMessage(field, message) {
        var _a;
        (_a = field.parentElement) === null || _a === void 0 ? void 0 : _a.classList.add(errorClass);
        const spanElement = field.nextElementSibling;
        spanElement.innerHTML = message;
    }
    function hideErrorMessage(field) {
        var _a;
        (_a = field.parentElement) === null || _a === void 0 ? void 0 : _a.classList.remove(errorClass);
    }
    function usernameValidator(username, usernameElement) {
        const isValid = username.length > 3;
        if (!isValid)
            showErrorMessage(usernameElement, 'Username must be at least 3 characters long');
        return isValid;
    }
    function emailValidator(email, emailElement) {
        const isValid = validator_1.default.isEmail(email);
        if (!isValid)
            showErrorMessage(emailElement, 'Email address must be valid');
        return isValid;
    }
    function passwordValidator(password, passwordElement) {
        const isValid = validator_1.default.isStrongPassword(password);
        if (!isValid)
            showErrorMessage(passwordElement, 'Password must be at least 6 characters long and must have letters, numbers or special characters');
        return isValid;
    }
    function passwordIsEqualValidator(password, password_confirmation, passwordConfirmationElement) {
        const isValid = validator_1.default.equals(password, password_confirmation);
        if (!isValid)
            showErrorMessage(passwordConfirmationElement, 'Password confirmation must be equal to password');
        return isValid;
    }
    form.addEventListener('submit', (event) => {
        event.preventDefault();
        const formSubmited = event.currentTarget;
        const username = formSubmited.querySelector('.username');
        const email = formSubmited.querySelector('.email');
        const password = formSubmited.querySelector('.password');
        const password2 = formSubmited.querySelector('.password2');
        const formData = new FormData(formSubmited);
        hideErrorMessage(username);
        hideErrorMessage(email);
        hideErrorMessage(password);
        hideErrorMessage(password2);
        if (usernameValidator(String(formData.get('username')), username) &&
            emailValidator(String(formData.get('email')), email) &&
            passwordValidator(String(formData.get('password')), password) &&
            passwordIsEqualValidator(String(formData.get('password')), String(formData.get('password2')), password2)) {
...
/******/ 	var __webpack_exports__ = __webpack_require__("./src/ValidateExercise/index.ts");
/******/ 	
/******/ })()
;
